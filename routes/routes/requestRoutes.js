 ...
 * - Si es diseñador/manager/admin: ve todas (con ?limit=N opcional)
 */
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    if (status) {
      // Solo roles autorizados pueden cambiar estado
      if (!['designer', 'manager', 'admin'].includes(req.user.role)) {
        return res.status(403).json({ success: false, message: 'Acceso denegado: no puede cambiar el estado' });
      }
